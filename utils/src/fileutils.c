...
int                             fprint_file(FILE *restrict out, FILE *restrict f){
    ...
    cnt += fprintf(out, "Origin:\n");
    while (strs[i].v != 0){
        cnt += fprintf(out, "%s", fsstr(strs[i]) );
        fs_free(&strs[i++]);
    }
...
bool                     fread_pattern(FILE *restrict f, const char *restrict pattern, int sz){
    ...
    if ( fread(buf, 1, sz, f) != (unsigned) sz){
        ...
        return userraise(false, ERR_NOT_ENOGH_VALUE, "Unable to read %d bytes from stream (%s)", sz, buf);
    }
    return memcmp(buf, pattern, sz) == 0;
}
...
tf2(const char *name)
{
    ...
    TFILE   tf;
    fs      s = fsempty();
    const char *fname = "res/fileutils_test_Readfs_file.out";
...
tf4(const char *name)
{
...

    {
        ...
        if (!out)
            return logacterr(fclose(f), TEST_FAILED, "Unable to open %s for write", outname);
        fprint_file(out, f);
        fclose(f);
        fclose(out);
    }
    return logret(TEST_MANUAL, "done"); // TEST_FAILED TEST_MANUAL ???
}
...
tf5(const char *name)
{
    ...
    test_sub("subtest %d: fread_pattern from test file", ++subnum);
    {
        ...
        for (int i = 0; i < 3; i++)
            fwrite(pattern, sizeof(pattern), 1, f);  // 3 times
        // try to read
        rewind(f);
        for (int i = 0; i < 3; i++)
            if (!freadpattern(f, pattern) )
